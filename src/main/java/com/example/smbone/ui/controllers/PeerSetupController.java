...
public class PeerSetupController {
    ...
    // ================= STEPS =================
    @FXML
    private StackPane step1, step2, step3, step4, step5, step6, step7, step8, step9, step10;

    @FXML
    private Region line1, line2, line3, line4, line5, line6, line7, line8, line9;
    @FXML
    ...
    private DoubleProperty progressValue = new SimpleDoubleProperty(0);
    ...
    public void initialize() {
        ...
        progressValue.addListener((obs, o, n) -> progressPercent.setText(String.format("%.0f%%", n.doubleValue() * 100)));
    ...
    // ================= STEP EXECUTION =================
    private void runStep(StackPane step, Region line, String msg, int stepNo) throws Exception {
        activate(step, msg);
    ...
    private void updateProgress(int step) {
        ...
        Platform.runLater(() -> {
            Timeline timeline = new Timeline(new KeyFrame(Duration.seconds(0.6), new KeyValue(progressValue, target, Interpolator.EASE_BOTH)));
            timeline.play();
        });
    }
    ...
    private void glow(StackPane step) {
        if (currentPulse != null) currentPulse.stop();
        currentPulse = new ScaleTransition(Duration.seconds(1.4), step);
        ...
        currentPulse.setCycleCount(Animation.INDEFINITE);
        currentPulse.play();
    }
    ...
    private void startGlassBackground() {
        ...
        GraphicsContext gc = glassCanvas.getGraphicsContext2D();

        // Auto resize
        glassCanvas.widthProperty().bind(((StackPane) glassCanvas.getParent()).widthProperty());

        glassCanvas.heightProperty().bind(((StackPane) glassCanvas.getParent()).heightProperty());
        ...
        AnimationTimer timer = new AnimationTimer() {
            ...
            public void handle(long now) {
                ...
                LinearGradient gradient = new LinearGradient(Math.sin(offset), 0, 1, Math.cos(offset), true, CycleMethod.NO_CYCLE,

                        new Stop[]{new Stop(0, Color.web("#1e293b")), new Stop(0.4, Color.web("#334155")), new Stop(0.7, Color.web("#0ea5e9")), new Stop(1, Color.web("#22d3ee"))});
